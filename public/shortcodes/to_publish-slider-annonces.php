...
function to_publish_slider_annonces( $atts ){
  ...
  if( count($posts) ):
    ...
    $return .= '<div class="div">'. __("Nouveautés","tanja-marina") .'</div>';
    $return .= '</div>';
    $return .= '<div class="section_sousTitle">';
    $return .= __("Découvrez toutes les dernières annonces","tanja-marina");
    $return .= '</div>';
      ...
      $return .= '<div class="annoce_container items">';
        $return .= '<a href="' . get_the_permalink() .'" title="'. esc_attr( get_the_title() ) .'">';
          ...
          $return .= '<div class="info_container">';
            ...
            if( $price ):
              $return .= '<div class="price">' . __("Price : ", "tanja-marina") . $price . 'dhs</div>';  
            endif;  
