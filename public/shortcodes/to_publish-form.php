...
function to_publish_do_frontend_form_submission_shortcode( $atts = array() ) {
	...
	if ( ( isset( $_GET['post_submitted'] ) && ( $post = get_post( absint( $_GET['post_submitted'] ) ) ) ) || (absint( $_GET['post_submitted'] ) == 123) ) {
		// Get submitter's name
		$name = get_post_meta( $post->ID, 'submitted_author_name', 1 );
		$name = $name ? ' '. $name : '';
		// Add notice of submission to our output
		$output .= '<div class="container"><div class="row"><div class="col-md-12"> <div class="alert alert-success">' . __( 'We have taken into account your request. We will contact you as soon as possible.', 'to_publish' ) . '</div></div></div></div>';
	} 
	// Get our form
